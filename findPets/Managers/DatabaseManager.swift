...
final class DatabaseManager {
    ...
    public func insertPost(
        with post : Posts, email : String, compl : @escaping(Bool) -> (Void)
    ){
        let userEmail = email
        ...
        let data : [String : Any] = [
            ...
            "title" : post.title,
            "body" : post.text,
            "created":post.timeWas,
            "headerImageURL" : post.headerImgURL?.absoluteString ?? ""
        ]
        database
            .collection("users")
            .document(userEmail)
    ...
    public func getPosts(
            ...
            database
                ...
                .getDocuments { snapshot, error in
                    ...
                    let posts: [Posts] = documents.compactMap({ dictionary in
                        ...
                        let post = Posts(
                            ...
                            timeWas: created,
                            text: body,
                            headerImgURL: URL(string: imageUrlString)
                        )
                        return post
                    })
    ...
    public func insertUser(user : User, compl : @escaping(Bool) -> (Void)){
    ...
    public func getProfile(email : String, compl : @escaping (User?) -> Void) {
        ...
        database.collection("users")
            ...
            .getDocument { snapshot, error in
                ...
                let user = User(name: name, email: email, profileImgRef: ref)
                compl(user)
               
            }
    }
    
    public func updateProfileImg(email : String,compl : @escaping (Bool)->Void) {
        let path = email
                    .replacingOccurrences(of: "@", with: "_")
                    .replacingOccurrences(of: ".", with: "_")
